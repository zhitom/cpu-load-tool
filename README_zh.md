...
| --hours                  | 工作时间段配置（见下方说明）              | （空）   |

### 工作时间段配置

`--hours` 参数允许您指定CPU负载应该处于活动或非活动状态的时间窗口。

**格式：** `in[x1,y1],ex[x2,y2]...`
- `in[x,y]`: 包含区间 - CPU负载在此期间处于活动状态
- `ex[x,y]`: 排除区间 - CPU负载在此期间处于非活动状态
- `x` 和 `y` 为0-23格式的小时数
...
**行为：**
- 仅使用`in[x,y]`：CPU负载仅在指定时间段内活动
- 仅使用`ex[x,y]`：CPU负载全天活动，除了指定时间段
- 同时使用两者：CPU负载在`in`时间段内活动，但在`ex`时间段内不活动
- 非工作时间：所有线程停止，不产生CPU负载
- 进入工作时间：所有线程恢复运行
- 离开工作时间：所有线程停止
- 如果未提供`--hours`参数，程序持续运行
